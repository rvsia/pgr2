...
char Load3DS (obj_type_ptr p_object, const char *p_filename)
{
    ...
    // prepisu indexy do samostatneho pole
    for (int i=0; i<MAX_POLYGONS; i++){
        p_object->indices[(i*3)] = p_object->polygon[i].a;
        p_object->indices[(i*3)+1] = p_object->polygon[i].b;
        p_object->indices[(i*3)+2] = p_object->polygon[i].c;
    }
