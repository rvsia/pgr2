...
void SceneWindow::initialize()
{
    ...
    int m_positionAttr = m_program->attributeLocation(vertexAtrName.c_str());
    int m_normalAttr = m_program->attributeLocation(normalAtrName.c_str());

    int m_projectionMatrixUniform = m_program->uniformLocation("projection_matrix");
    int m_modelViewMatrixUniform = m_program->uniformLocation("modelview_matrix");
...
void SceneWindow::render()
{
    ...
    m_program->setUniformValue("light0.spot_direction", 1.0f, -1.0f, -7.0f);
    m_program->setUniformValue("light0.spot_cut_off", 0.90f);
    m_program->setUniformValue("light0.spot_exponent", 2.0f);

    m_program->setUniformValue("light0.constant_attenuation", 0.0f);
    m_program->setUniformValue("light0.linear_attenuation", 0.03f);
